...
class FilterNet2(nn.Module):
    def __init__(self,gen_data_config, model_config, train_config):
        ...
        # self.k = model_config['em_d']
        # self.is_bidirectional = model_config['is_bid']
        self.Ann1 = RNNBlock(self.input_dim, self.seq_len, self.k, self.is_bidirectional)
        self.Xnn2 = RNNBlock(self.input_dim, self.seq_len, self.k, self.is_bidirectional)
        if self.is_bidirectional:
            self.k = 2 * self.k
        self.Bnn3 = RNNBlock(self.n, self.seq_len, self.k)
        ...
        # )
        self.fc = FeatureWiseLinear(self.seq_len, self.k)
        self.W_v = nn.Linear(self.k, self.n)
        self.W_k = nn.Linear(self.k, self.n)
        self.W_q = nn.Linear(self.k, self.n)
    ...
    def forward(self, X):
        ...
        embed_P = B.permute(0, 2, 1) #out B (batch_size, n, k)
        P = self.fc(embed_P) #out B (batch_size, n, 2 * m)
...
def filterNet2_train(num_epochs,model,optimizer,train_loader,val_loader, criterion, device,writer,folder_path,scheduler,early_stop):
    # 训练循环
    best_ccc = 0.0
    not_improve_epoch = 0
...
def filterNet2_eval(model, val_loader, device, validate_config,gen_data_config, folder_path):
