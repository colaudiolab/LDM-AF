...
class FilterNet6(nn.Module):
    def __init__(self,gen_data_config, model_config, train_config):
        ...
        # 添加可学习参数 A
        self.A = nn.Parameter(torch.zeros(self.input_dim, self.w))  # 零初始化
        ...
        self.W_v = nn.Linear(self.w, self.n)
        self.W_k = nn.Linear(self.w, self.n)
...
def filterNet6_train(num_epochs,model,optimizer,train_loader,val_loader, criterion, device,writer,folder_path,scheduler,early_stop):
    # 训练循环
    best_ccc = 0.0
    not_improve_epoch = 0
    for epoch in range(num_epochs):
        ...
        for batch in pbar:
            ...
            labels = labels[:,train_loader.dataset.l:train_loader.dataset.l+train_loader.dataset.w]
...
def filterNet6_eval(model, val_loader, device, validate_config,gen_data_config, folder_path):
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader):
            signals = batch['datas_t'].to(device)
            labels = batch['labels'].to(device)
            labels = labels[:, val_loader.dataset.l:val_loader.dataset.l + val_loader.dataset.w]
