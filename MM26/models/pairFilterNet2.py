...
class FilterBlock(nn.Module):
    def __init__(self,gen_data_config, model_config, train_config):
        ...
        self.selector = nn.Parameter(torch.ones(self.input_dim, self.seq_len) * 0.5413248546129181) # 配合softPLUS 1初始化
    ...
    def forward(self, X):
        X_orig = X.clone()
        Selector_expanded = F.softplus(self.selector).unsqueeze(0).expand(X.size(0), -1, -1)
        X = X * Selector_expanded
...
class PairFilterNet2(nn.Module):
    def __init__(self,gen_data_config, model_config, train_config):
        #in_dim 2*m
        #mid_dim n
        #out_dim 2*m
...
def pairFilterNet2_train(num_epochs,model,optimizer,train_loader,val_loader, criterion, device,writer,folder_path,scheduler,early_stop, group_name):
    # 训练循环
    best_ccc = 0.0
    not_improve_epoch = 0
...
def pairFilterNet2_eval(model, val_loader, device, validate_config,gen_data_config, folder_path):
