...
def run_main(root_path, gen_data_config, train_config, model_config,validate_config,only_run_validate,is_save_config = True):
    # 创建文件夹
    folder_name = ""
    for key, value in gen_data_config.items():
        # 添加键值对到文件名
        if key != "modal_dim" and key != "tr_dir" and key != "va_dir" and key != "te_dir":
    ...
    folder_path = Path(root_path) / folder_name.rstrip("_")
    ...
    for group_name, scaler in zip(groups, scalar_list):
        outputs, labels, val_ccc, embed_P = val_main(gen_data_config, train_config,model_config,validate_config,folder_path, group_name, scaler)
        logmsg(folder_path / 'printLogs.txt', f"{group_name} eval CCC: {val_ccc}")
    ...
    logmsg(folder_path / 'printLogs.txt', f"total eval CCC: {val_ccc}")

    return embed_P, val_ccc
...
if __name__ == '__main__':
    ...
    run_main(root_path, gen_data_config, train_config, model_config,validate_config,only_run_validate,is_save_config=True)