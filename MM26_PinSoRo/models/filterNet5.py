...
class FilterNet5(nn.Module):
    def __init__(self,gen_data_config, model_config, train_config):
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.n, 2 * self.n),
            nn.ReLU(),
            nn.Linear(2 * self.n, 2 * self.n),
            nn.ReLU(),
            nn.Linear(2 * self.n, 2 * self.n),
            nn.ReLU(),
            nn.Linear(2 * self.n, 100),
        )
...
def filterNet5_train(num_epochs,model,optimizer,train_loader,val_loader, criterion, device,writer,folder_path,scheduler,early_stop):
    # 训练循环
    best_ccc = 0.0
    not_improve_epoch = 0
    for epoch in range(num_epochs):
        ...
        for batch in pbar:
            ...
            labels = torch.floor(labels * 100).long()
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader):
                ...
                outputs = (outputs.float() + 0.5) / 100.0
...
def filterNet5_eval(model, val_loader, device, validate_config,gen_data_config):
