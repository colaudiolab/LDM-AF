...
def train_main(gen_data_config, train_config,model_config,folder_path, group_name, only_run_validate = False):
    ...
    train_dataset = MM26Dataset_pair(task = gen_data_config['task'], modal = gen_data_config['modal'], modal_dim = gen_data_config['modal_dim'], dir_path  = gen_data_config['tr_dir'], group_name = group_name, w = gen_data_config['w'], l = gen_data_config['l'], s = gen_data_config['s'])
    ...
    scaler.partial_fit(train_dataset.datas_e)
    ...
    existing_classes = [int(c) for c in np.unique(train_dataset.labels)]
    ...
    full_weights = np.ones(4 if gen_data_config['task'] == 'task' else 5, dtype=np.float32)
    for i, c in enumerate(existing_classes):
        full_weights[c] = class_weights[i]
