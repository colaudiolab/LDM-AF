...
def val_main(gen_data_config, train_config,model_config,validate_config,folder_path, group_name, scaler):
    ...
    val_dataset = MM26Dataset_pair(task = gen_data_config['task'],modal = gen_data_config['modal'], modal_dim = gen_data_config['modal_dim'], dir_path  = gen_data_config['va_dir'], group_name = group_name, w = gen_data_config['w'], l = gen_data_config['l'], s = gen_data_config['w'])
    ...
    #     filterNet6_train(num_epochs,model,optimizer,train_loader,val_loader, criterion, device,writer,folder_path,scheduler,early_stop)
        ...
        outputs_list, labels_list, val_f1,accuracy,precision,recall,embed_P = pairFilterNet1_eval(model, val_loader, device, validate_config, gen_data_config, folder_path)
    elif train_config['model'] == 'PairFilterNet2':
        model = PairFilterNet2(gen_data_config, model_config, train_config).to(device)
        model.load_state_dict(torch.load(folder_path / f'{group_name}_best_model.pth', map_location=device))
        outputs_list, labels_list, val_f1,accuracy,precision,recall,embed_P = pairFilterNet2_eval(model, val_loader, device, validate_config, gen_data_config, folder_path)
    elif train_config['model'] == 'PairFilterNet3':
        model = PairFilterNet3(gen_data_config, model_config, train_config).to(device)
        model.load_state_dict(torch.load(folder_path / f'{group_name}_best_model.pth', map_location=device))
        outputs_list, labels_list, val_f1,accuracy,precision,recall,embed_P = pairFilterNet3_eval(model, val_loader, device, validate_config, gen_data_config, folder_path)
    elif train_config['model'] == 'PairFilterNet4':
        model = PairFilterNet4(gen_data_config, model_config, train_config).to(device)
        model.load_state_dict(torch.load(folder_path / f'{group_name}_best_model.pth', map_location=device))
        outputs_list, labels_list, val_f1,accuracy,precision,recall,embed_P = pairFilterNet4_eval(model, val_loader, device, validate_config, gen_data_config, folder_path)
    elif train_config['model'] == 'PairFilterNet5':
        model = PairFilterNet5(gen_data_config, model_config, train_config).to(device)
        model.load_state_dict(torch.load(folder_path / f'{group_name}_best_model.pth', map_location=device))
        outputs_list, labels_list, val_f1,accuracy,precision,recall,embed_P = pairFilterNet5_eval(model, val_loader, device, validate_config, gen_data_config, folder_path)

    return outputs_list, labels_list, val_f1,accuracy,precision,recall,embed_P
